...
Issues and specs for this repo live as markdown files in `.scratch/`. This repo has no
remote, so there is no GitHub or GitLab issue tracker to write to.
...
If this repo gains a GitHub remote, re-run `setup-matt-pocock-skills` to switch. Existing
`.scratch/` tickets are not migrated automatically.
